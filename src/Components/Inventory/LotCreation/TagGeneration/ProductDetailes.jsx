...
const ProductDetails = ({resetFields, resetTrigger, setResetTrigger, updateTotals, mname, productNameRef, lotno, counter, prefix, manufacturer, dealername, tagInfo, feachTagno }) => {
  ...
  useEffect(() => {
    if (resetTrigger) {
      // Reset all states when resetTrigger is true
      setSelectedProduct('');
      setSelectedProductCode('');
      setSelectedProductCategory('');
      setSelectedProductHSNCode('');
      setPcs(1);
      setGwt(0);
      setBreadsLess(0);
      setTotalLess(parseFloat(localStorage.getItem("finalTotalGrams")) || 0);
      setNwt(0);
      setFileList([]);
      setResetTrigger(false); // Reset trigger back to false
    }
  }, [resetTrigger]);
